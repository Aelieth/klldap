...
A complete configuration for **SSSD** against KLLDAP using a hybrid **LDAP + Kerberos**
model:

- **Identity (users, groups, POSIX attributes)**: provided by LDAP (KLLDAP)
- **Authentication**: provided by Kerberos (KLLDAP's KDC)
...
Placeholders used below: realm `EXAMPLE.COM`, base DN `dc=example,dc=com`, KLLDAP host
`192.0.2.10`. Replace them with yours; the realm is derived from your base DN
(`dc=example,dc=com` → `EXAMPLE.COM`) unless `LLDAP_KERB_REALM_NAME` overrides it.

---

## 1. Prerequisites (Fedora; adjust to your distro)
...
Proper `krb5.conf` configuration is essential for both SSSD and browser SPNEGO to work
correctly.

### 2.1 Recommended `/etc/krb5.conf`
...
### 2.2 Alternative: explicit encryption types
...
This version explicitly prefers **AES256** encryption, which helps avoid the "Cannot
find key of appropriate type" errors with Java applications like Keycloak.

KLLDAP's image publishes the KDC (88) and kadmin (749) ports but not kpasswd (464):
passwords are changed through KLLDAP (web UI, GraphQL, or LDAP), which updates the KDC.
Leave `kpasswd_server` out.
...
[domain/klldap]
    ...
    auth_provider = krb5
    krb5_server = 192.0.2.10
    krb5_realm = EXAMPLE.COM
    krb5_validate = false
    krb5_store_password_if_offline = true

    # Password changes go through KLLDAP's LDAP password-modify operation
    # (RFC 3062, self-service), which also updates the KDC principal;
    # kpasswd is not published by the image.
...
[domain/klldap]
    # === Identity from LDAP ===
    id_provider = ldap
    ldap_uri = ldaps://ldap.example.com:6360          # Use ldaps:// for LDAPS
    ldap_search_base = dc=example,dc=com
    ldap_user_search_base = ou=people,dc=example,dc=com
    ldap_group_search_base = ou=groups,dc=example,dc=com
...
- Use a dedicated bind user with minimal privileges (a member of `lldap_strict_readonly`).
...
```bash
# Allow the custom LDAP ports (3890 plain, 6360 LDAPS)
...
This section covers integrating your KLLDAP + Kerberos setup with **Keycloak** for
web-based SSO using SPNEGO (Kerberos).

### 7.1 Docker Networking & Volume Recommendations

For the best experience:

- Run both the **KLLDAP** and **Keycloak** containers on the **same Docker host** and
  preferably on the **same Docker network**.
...
If the containers are on different machines, you will need to securely copy the keytab
and update the path in Keycloak.

### 7.2 Create a Dedicated Bind User (Recommended)

Instead of using the admin account, create a low-privilege user for Keycloak:

1. Create a new user in KLLDAP (e.g. `keycloak`).
2. Add this user to the **`lldap_strict_readonly`** group.

This user will be used by Keycloak for LDAP searches. It is much more secure than using
the admin account.
...
2. **Test Connection** (left side):
   ...
   - The admin password can come from the environment variable
     `LLDAP_KEYCLOAK_ADMIN_PASS`.
...
4. Fill in:
   ...
   - **LLDAP URL** (e.g. `ldap://klldap:3890` or external address)
   - **Sync Username** → the dedicated user you created in the `lldap_strict_readonly`
     group
   - **Sync Password**
   - Optional: Enable HSTS and Brute Force Protection
5. Click the buttons:
   - **Export keytab** — creates/updates the Keycloak service keytab in the shared
     volume (`/data/keytab/keycloak-http.keytab`, principal `HTTP/<hostname>@REALM`).
   - **Push To Keycloak** — creates a new realm in Keycloak pre-configured with your
     KLLDAP as an LDAP + Kerberos User Federation provider, along with sensible mappers.

> **Note**: After pushing, you will likely want to review and adjust settings inside
...
2. Set the following preferences:
   - `network.negotiate-auth.trusted-uris` → `.example.com, keycloak.example.com`
   - `network.negotiate-auth.delegation-uris` → `.example.com, keycloak.example.com`

#### Chrome / Chromium / Edge

These browsers usually inherit SPNEGO settings from the operating system. On Linux you
can launch with:
...
```
http://keycloak.example.com:8080/realms/your-realm/account
```

If everything is configured correctly, you should be logged in automatically without
...
```bash
klist
curl -v --negotiate -u : http://keycloak.example.com:8080/realms/your-realm/account
```
...
| User lookup works but authentication fails  | Kerberos misconfiguration, or the user has no principal (`kerberosSync` off, or no password set since) | Verify `krb5.conf` and `krb5_server`; `kadmin.local -q listprincs` in the container |
...
ldapsearch -x -H ldap://192.0.2.10:3890 -b "dc=example,dc=com" -s base
...
- Prefer `access_provider = ldap` over `permit` when possible for better access
  control.
...
# Check everything
sssctl domain-status klldap && getent passwd testuser && klist
```
...
|--------------------------------------|--------------------------------|--------|
| `default_realm`                      | `EXAMPLE.COM`                  | Your Kerberos realm |
